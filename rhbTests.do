cap drop _all
import excel "/Users/rhbob/Desktop/stata/annotated_v5_ALL_drivesFixedForStataOnly.xlsx", sheet("annotated_v5_ALL_drivesFixed") firstrow clear
keep CandID-WC Ability posemo negemo drivesCorr Standout Grindstone Agentic Communal
...
label var drivesCorr "Drive Corrected Words"
...
mac drop _all
foreach x in WC posemo negemo drivesCorr Standout Grindstone Agentic Communal Ability{
...
cibar drivesCorr, over1(can) over2(writer) over3(disc) barcol(ltblue erose ) blsize(tiny) graphopts(ytitle("Drive Corrected Words")legend(off))
graph export fig1_drives.eps,replace 
...
graph combine fig1_WC.gph fig1_agentic.gph fig1_communal.gph fig1_drives.gph fig1_grindstone.gph  fig1_standout.gph fig1_posemo.gph fig1_negemo.gph, rows(4) cols(2) 
 
graph save "~rhbob/Documents/fig1_drives",replace
...
mac drop _all
foreach x in WC posemo negemo drivesCorr Standout Grindstone Agentic Communal {
	ttest `x' if discipline==0,by(can)
